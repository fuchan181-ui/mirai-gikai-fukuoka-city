...
同じイベント・議決段階の金額でも、款別と節別のように集計の軸が違うことがある。
`fiscal_amount_sets.amount_set_key`が軸を表し、`(fiscal_event_id,
decision_stage, amount_set_key)`で1つの金額セットとする。予算概要の款別
（`purpose`と`revenue_source`を1つの表に並べる）は`primary`、歳出予算節別集計表の
節別は`section`とする。軸ごとに改訂系列と公開版を1つ持ち、別の軸の公開版を
置き換えない。取込は資料の金額候補の`classificationScheme`から軸を決め、款別と
別の軸が1つのグループに混ざる候補は軸を決められないため失敗させる。
同じ資料版から別の軸を取り込む場合も、根拠の出現を軸ごとに分けていないため
失敗させる。
公開画面は読む軸を明示し、軸をまたいで合計しない。
...
令和5〜8年度の歳出予算節別集計表parserは、歳出の節27行と予備費を千円精度で抽出する。
表は2ページに分かれ、1ページ目に款1〜6と節名、2ページ目に款7〜13と合計列が並ぶ。
節名と金額が別ページになるため、款別額が一致する組み合わせとして一意に整列できた
ときだけ値を採用し、全額が0の節の印字を省略する年度は0千円として補う。節は款とは
別の集計軸なので`fiscal_classifications.scheme = 'section'`へ保存する。公開画面は
款と歳入款以外のschemeを読み飛ばして款別集計へ混ぜず、節別の表示は後続PRで扱う。
