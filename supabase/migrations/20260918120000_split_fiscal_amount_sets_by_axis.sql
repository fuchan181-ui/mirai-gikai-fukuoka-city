-- 集計軸の違う金額を、同じイベント・議決段階に並べて公開できるようにする。
--
-- 歳出予算節別集計表（s-6）の節別金額は、予算概要の款別金額と同じ
-- イベント・議決段階（当初予算・提案）に属する。しかし fiscal_amount_sets は
-- (fiscal_event_id, decision_stage) で一意だったため、款別の金額セットを
-- 公開済みの年度では節別を反映できなかった。
--
-- 金額セットのidentityへ集計軸 (amount_set_key) を足し、軸ごとに独立した
-- 改訂系列を持たせる。既存の金額セットは款別（purpose と revenue_source を
-- 1つの表に並べる軸）なので primary とする。
--
-- 同じsource occurrence・source version・parse run・evidence revisionを別の
-- 対象revisionへ再割当しない契約は変えない。軸を分けても、節別の根拠は
-- 節別の金額セット改訂へだけ結び付く。
--
-- apply_verified_fiscal_staging は、同じ資料から取り込む候補のあいだで
-- 集計軸を判定する。
--   - 候補の scheme が1つだけ: purpose / revenue_source は款別 (primary)、
--     それ以外はその scheme（節別なら section）を軸にする。
--   - 候補の scheme が款別のものだけで複数: 款別 (primary)。
--   - 款別と別の軸が1つのグループに混ざるバッチ: 軸を決められないため失敗。
--
-- 同じ資料版を新しい解析で取り込み直す差し替えの順序（観測を superseded に
-- してから新しい観測で作り直す）は、そのまま残す。

alter table public.fiscal_amount_sets
  add column amount_set_key text not null default 'primary'
    check (nullif(btrim(amount_set_key), '') is not null);

-- 既定値は既存行を款別へ寄せるためのもの。書き込み側は軸を明示する。
-- 既定に頼ると、公開画面が合計する款別 (primary) へ黙って混ざる。
comment on column public.fiscal_amount_sets.amount_set_key is
  '同じイベント・議決段階に並ぶ集計軸。款別は primary、節別は section。';

alter table public.fiscal_amount_sets
  drop constraint fiscal_amount_sets_fiscal_event_id_decision_stage_key;

alter table public.fiscal_amount_sets
  add constraint fiscal_amount_sets_fiscal_event_id_decision_stage_axis_key
    unique (fiscal_event_id, decision_stage, amount_set_key);
...
begin
  ...
  loop
    ...
    -- 集計軸は、同じグループの金額候補の scheme から決める。款別は
    -- purpose と revenue_source を1つの表に並べるため primary にまとめ、
    -- 節別のような別の軸は scheme 名をそのまま軸にする。
    select
      count(distinct coalesce(
        nullif(btrim(coalesce(
          record.parsed_payload ->> 'classificationScheme', ''
        )), ''),
        'purpose'
      )),
      min(coalesce(
      ...
      coalesce(bool_or(coalesce(
        ...
        'purpose'
      ) not in ('purpose', 'revenue_source')), false)
    into v_scheme_count, v_scheme, v_has_other_scheme
    from public.fiscal_staging_records record
    where record.batch_id = p_batch_id
      and record.record_kind = 'amount'
      and record.parsed_payload ->> 'eventKind' = v_group.event_kind
    ...
    if coalesce(v_scheme_count, 0) = 0 then
      raise exception 'fiscal staging apply requires amount candidates';
    end if;
    -- 款別（purpose と revenue_source）だけで複数の scheme があるのは、
    -- 歳入と歳出を1つの表に並べた款別の資料なので primary にまとめる。
    -- 款別以外の scheme が混ざると、どの軸の金額か決められない。
    if v_scheme_count > 1 and v_has_other_scheme then
      raise exception
        'fiscal staging apply cannot mix aggregation axes in one amount set';
    end if;

    if v_scheme_count = 1
      and v_scheme not in ('purpose', 'revenue_source') then
      v_amount_set_key := v_scheme;
    else
      v_amount_set_key := 'primary';
    end if;
    ...
    where amount_set.fiscal_event_id = v_event_id
      ...
      and amount_set.amount_set_key = v_amount_set_key;

    if v_amount_set_id is null then
      insert into public.fiscal_amount_sets (
        fiscal_event_id, event_kind, decision_stage, amount_set_key
      ) values (
        v_event_id, v_event_kind, v_decision_stage, v_amount_set_key
      )
    ...
    -- source_amount_set_key は資料版の出現を指す安定キーで、集計軸は含めない。
    -- 同じ資料版から別の軸を取り込むと別の金額セットの occurrence を再利用して
    -- しまうため、黙って結び付けず失敗させる。
    select occurrence.id, occurrence.amount_set_id
      into v_amount_set_occurrence_id, v_occurrence_amount_set_id
    ...
    if v_amount_set_occurrence_id is not null
      and v_occurrence_amount_set_id <> v_amount_set_id then
      raise exception
        'fiscal staging apply cannot reuse one edition occurrence across aggregation axes';
    end if;
...
comment on function apply_verified_fiscal_staging(uuid, uuid) is
  '検証済みの財政stagingを資料版・集計範囲・分類・金額の公開正本へ反映する。金額候補の集計軸ごとに金額セットを分け、同じ資料版の取り込み直しにも対応する。';
